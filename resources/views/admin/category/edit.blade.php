...
<section class="content">
  <div class="container-fluid">
    ...
    <div id="accordion">
      <!-- form start -->
      <form action="{{route('admin.category.update', $category->id)}}" method="post" name="form-example-1" id="form-example-1" enctype="multipart/form-data">
        @csrf
        @METHOD('PUT')
        <!-- card common start -->
        <div class="card">
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select name="section_id" id="section_id" class="form-control">
                      ...
                      <option value="{{$section->id}}" {{$category->section->id==$section->id ? ' selected': ''}}>{{$section->title}}</option>
                      @endforeach
                      @else
                      <option value=""> No sections Create one Now</option>
                      @endif
                    </select>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    ...
                    <select name="parent_id" id="parent_id" class="form-control">
                      <option value="{{$category->parent_id}}">{{$category->parent_id ? $category->parent->title : 'top Level'}}</option>
                    </select>
                  </div>
...
<script>
  ...
  $(document).ready(function() {
    const CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    const serverResponse = document.getElementById('parent_id');
